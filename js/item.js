  ...
  window.projectData.items.forEach((item, index) => {
    ...
    const typeBadge =
      item.type === "consumable"
        ? `<span class="bg-orange-100 text-orange-600 text-xs px-2 py-1 rounded whitespace-nowrap font-bold">消耗品</span>`
        : `<span class="bg-purple-100 text-purple-600 text-xs px-2 py-1 rounded whitespace-nowrap font-bold">永久道具</span>`;
    ...
    headerEl.innerHTML = `
      <div class="flex items-center space-x-3 w-full">
        ...
        <input type="text" value="${item.name}" placeholder="輸入道具名稱..." 
               class="w-full max-w-[500px] font-bold text-lg ${titleColor} bg-transparent border border-transparent hover:bg-white hover:border-gray-300 hover:shadow-sm focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none rounded px-2 py-1 transition-all cursor-text">
      </div>
      <div class="flex items-center space-x-2 ml-4">
        ${typeBadge}
        <button class="delete-btn text-red-500 hover:text-red-700 p-1 bg-white hover:bg-red-50 rounded shadow-sm border border-transparent hover:border-red-200 transition" title="刪除此道具">
